...
begin
  if FState <> wsDisabled then
    FState := wsHover;
end;

procedure TBaseWidget.OnHoverLeave;
begin
  if FState = wsHover then
    FState := wsNormal;
end;

procedure TBaseWidget.OnFocusGained;
begin
  if FState <> wsDisabled then
    ; // потом можно добавить визуализацию
end;

procedure TBaseWidget.OnFocusLost;
begin
  if FState <> wsDisabled then
    ; // потом можно добавить визуализацию
end;
...
procedure TBaseWidget.Invalidate;
begin
  FDirty := True;
end;

function TBaseWidget.IsDirty: Boolean;
begin
  Result := FDirty;
end;

function TBaseWidget.IsVisible: Boolean;
begin
  Result := FVisible;
end;

procedure TBaseWidget.SetVisible(value: Boolean);
begin
  FVisible := value;
end;
